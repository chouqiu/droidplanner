...
public class MySurfaceView extends SurfaceView  implements Callback, Runnable {
    ...
    */
	...
	public MySurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
        final NovaPlannerApp nvapp = (NovaPlannerApp)context.getApplicationContext();
        drone = nvapp.getDrone();
	...
	{
        final float OFFSET_Y = 350;
        LEFT_CENTERX = Height / 2;
        LEFT_CENTERY = Height / 2 + OFFSET_Y;
        RIGHT_CENTERX = screenWidth - LEFT_CENTERX;
        RIGHT_CENTERY = Height / 2 + OFFSET_Y;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        try {
            ...
            /*
            if (altCtrlMode == 0)
                throttle = (int) (1000 + 1000 * (BackRectButtom - SmallRockerCircleY) / (BackRectButtom - BackRectTop));
            else
                throttle = (int) (1500 - 1000 * (SmallRockerCircleY - leftTouchStartY) / (BackRectButtom - BackRectTop));
            */
            int base = 1000, weight=2000, pitch_base = 1500, pitch_weight = 1000;

            /*throttle*/pitch = (int) (pitch_base + pitch_weight * (0-(SmallRockerCircleY - leftTouchStartY)) / (BackRectButtom - BackRectTop));
            yaw = (int) (base + weight * ((SmallRockerCircleX - leftTouchStartX)) / (BackRectRight - BackRectLeft));
            //pitch = (int) (1500 + 2000 * (0 - (SmallRockerCircleY2 - rightTouchStartY)) / (BackRectButtom2 - BackRectTop2));
            throttle = (int) (base + weight * (0-(SmallRockerCircleY2 - rightTouchStartY)) / (BackRectButtom2 - BackRectTop2));
            roll = (int) (base + weight * ((SmallRockerCircleX2 - rightTouchStartX)) / (BackRectRight2 - BackRectLeft2));

            if ( drone.getControls().touchReadyToSend == false ) {
                drone.getControls().throttle = (short)constrainRange(throttle, -1000, 1000);
                drone.getControls().yaw = (short)constrainRange(yaw, -1000, 1000);
                drone.getControls().pitch = (short)constrainRange(pitch, -1000, 1000);
                drone.getControls().roll = (short)constrainRange(roll, -1000, 1000);

                drone.getControls().touchReadyToSend = true;
            }

            Log.i(TAG, "yaw: " + Integer.toString(yaw)
                    + " trottle: " + Integer.toString(throttle)
    ...
    public void tryRequestLayout(boolean isOpen) {
        if ( isOpen == true ) {
            isSuspend = true;
            clearDraw();
        } else {
